...
def BuildBootableImage(sourcedir, fs_config_file):
  ...
  uramdisk = tempfile.NamedTemporaryFile()
  ...
  cmd = ["mkimage", "-A", "arm", "-T", "ramdisk", "-C", "none", "-n", "ramdisk", "-a", "0x40800000"]
  cmd.append("-d")
  cmd.append(ramdisk_img.name)
  cmd.append(uramdisk.name)
  p3 = Run(cmd)  
  p3.wait()
  p2.wait()
  p1.wait()
  ...
  assert p3.returncode == 0, "mkimage of %s ramdisk failed" % (targetname,)
  ...
  if os.access(fn, os.F_OK):
    ...
    cmd.extend(["--ramdisk", uramdisk.name,
                "--output", img.name])
